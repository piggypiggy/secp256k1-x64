...
$code.=<<___;
################################################################################
# void secp256k1_avx2_gather_w5(uint64_t *val, uint64_t *in_t, int index);
.type	secp256k1_avx2_gather_w5,\@abi-omnipotent
.align	32
secp256k1_avx2_gather_w5:
.Lavx2_gather_w5:
    vzeroupper
___
$code.=<<___	if ($win64);
    lea	-0x88(%rsp), %rax
.LSEH_begin_secp256k1_avx2_gather_w5:
...
$code.=<<___	if ($win64);
    ...
    lea	0xa8(%rsp), %rsp
.LSEH_end_secp256k1_avx2_gather_w5:
___
$code.=<<___;
    ret
.size	secp256k1_avx2_gather_w5,.-secp256k1_avx2_gather_w5
___
}
if ($avx>1) {
my ($val,$in_t,$index)=$win64?("%rcx","%rdx","%r8d"):("%rdi","%rsi","%edx");
...
################################################################################
# void secp256k1_avx2_gather_w7(uint64_t *val, uint64_t *in_t, int index);
.globl	secp256k1_avx2_gather_w7
.type	secp256k1_avx2_gather_w7,\@abi-omnipotent
.align	32
secp256k1_avx2_gather_w7:
.Lavx2_gather_w7:
    vzeroupper
___
$code.=<<___	if ($win64);
    lea	-0x88(%rsp), %rax
.LSEH_begin_secp256k1_avx2_gather_w7:
    .byte	0x48,0x8d,0x60,0xe0		#lea	-0x20(%rax), %rsp
    .byte	0xc5,0xf8,0x29,0x70,0xe0	#vmovaps %xmm6, -0x20(%rax)
    .byte	0xc5,0xf8,0x29,0x78,0xf0	#vmovaps %xmm7, -0x10(%rax)
    .byte	0xc5,0x78,0x29,0x40,0x00	#vmovaps %xmm8, 8(%rax)
...
$code.=<<___	if ($win64);
    ...
    movaps	0x90(%rsp), %xmm15
    lea	0xa8(%rsp), %rsp
.LSEH_end_secp256k1_avx2_gather_w7:
___
$code.=<<___;
    ret
.size	secp256k1_avx2_gather_w7,.-secp256k1_avx2_gather_w7
___
} else {
$code.=<<___;
.globl	secp256k1_avx2_gather_w7
.type	secp256k1_avx2_gather_w7,\@function,3
.align	32
secp256k1_avx2_gather_w7:
    .byte	0x0f,0x0b	# ud2
    ret
